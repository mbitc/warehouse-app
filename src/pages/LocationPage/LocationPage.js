...
const LocationPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/locations?_embed=storages`)
  ...
  const inputLocationElement =
    addLocationInput || editLocationInput ? (
  ...
  const locationOptionsElement = location.map((place) => {
    return (
      <option key={place.id} value={place.id}>
        {place.line}-{place.sector}-{place.level}
      </option>
    );
  });
  ...
  const locationListElement = location.map((place) => {
    return (
      <li key={place.id} className={style.locationList}>
        <button
          className='delete'
          onClick={() => deleteLocationHandler(place.id)}
        />
        <button
          className='edit'
          onClick={() => editLocationHandler(place.id)}
        />
        <span>
          {place.line}-{place.sector}-{place.level}
        </span>
  ...
  const createLocationHandler = () => {
    if (addLocationInput) {
      axios
        .post(`${API_URL}/locations`, place)
        .then((res) => {
          setLocation((prevState) => [...prevState, res.data]);
          toast.success(
            `Location ${place.line}-${place.sector}-${place.level} is ${res.statusText} `
          );
        })
        .catch((err) => toast.error(err.message));
  ...
  const saveLocationHandler = () => {
    axios
      .patch(`${API_URL}/locations/${place.id}`, place)
      .then((res) => {
        toast.success(
          `Location ${place.line}-${place.sector}-${place.level} is ${res.statusText} `
        );
  ...
  const deleteLocationHandler = (id) => {
    axios
      ...
      .then((res) => {
        const locationIndex = location.findIndex(
          (place) => place.id === Number(id)
        );
        setLocation((prevState) => prevState.toSpliced(locationIndex, 1));
        toast.info(
          `Location ${place.line}-${place.sector}-${place.level} is deleted ${res.statusText} `
        );
        setAddLocationInput(false);
        setEditLocationInput(false);
      })
  ...
  return (
    <Container>
      ...
      {!editLocationInput && (
        <button className='btn long' onClick={createLocationHandler}>
          {addLocationInput ? 'Add' : 'Create Location'}
        </button>
      )}
      {editLocationInput && (
        <button className='btn long' onClick={saveLocationHandler}>
          Save
        </button>
      )}
      <button className='btn long' onClick={stackingItemsHandler}>
        {addItemsInput ? 'Stacking' : 'Stack Items'}
      </button>
