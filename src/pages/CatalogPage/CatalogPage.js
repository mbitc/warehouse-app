...
const CatalogPage = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/catalogs?_embed=products`)
  ...
  const saveCatalogHandler = () => {
    axios
      .patch(`${API_URL}/catalogs/${catalog.id}`, catalog)
      .then((res) => {
  ...
  const catalogListElement = catalogs.map((catalog) => {
    return (
      <li key={catalog.id} className={style.categoryList}>
        ...
        <button
          className='edit'
          onClick={() => editCatalogHandler(catalog.id)}
        />
        <Link
          className='link'
          key={catalog.id}
          to={`${ROOT_PATH}catalog/${catalog.id}`}
        >
  ...
  const inputElement =
    addCatalogInput || editCatalogInput ? (
  ...
  return (
    <Container>
      {inputElement}
      {!editCatalogInput && (
        <button className='btn long' onClick={createCatalogHandler}>
          {addCatalogInput ? 'Add' : 'Create Catalog'}
        </button>
      )}
      {editCatalogInput && (
        <button className='btn long' onClick={saveCatalogHandler}>
          Save
        </button>
      )}
